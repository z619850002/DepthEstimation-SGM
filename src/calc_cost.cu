                 ...
                 int nMinDep, int nMaxDep)
{
    ...
    if (tidx >= 0 && tidx < WIDTH && tidx >= 0 && tidy < HEIGHT)
    {
        ...
        {
            if (d >= nMinDep-1 && d + i < nMaxDep && c_min[d + i] < c_min[d])
            {
                c_min[d] = c_min[d + i];
                c_idx[d] = c_idx[d + i];
            }
        ...
        if (threadIdx.x == 0)
        {
            float min_cost = c_min[nMinDep-1];
            int min_idx = c_idx[nMinDep-1];

            if (tidx == 130 && tidy == 70){
                printf("min_idx is: %d\n", min_idx);
                printf("nMinDep is %d, nMaxDep is %d", nMinDep, nMaxDep);
